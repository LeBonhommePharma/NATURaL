...
public enum BrandColor: Sendable {
    ...
    //
    // These resolve through BrandColors.xcassets, which as of 20 September 2026
    // genuinely carries light/dark pairs — generated by the canonical
    // design-system session and consumed here, not authored locally. Each
    // colorset holds the light value in `universal` and the dark value under
    // `luminosity: dark`, which is the direction Apple resolves; the catalog
    // previously held the dark value in `universal` with no variant, so light
    // appearance would have rendered dark-on-dark had anything removed the
    // pinned `.preferredColorScheme(.dark)`.
    //
    // The literals above (BrandPalette / BrandColor) are fixed sRGB and do NOT
    // adapt. That is deliberate: MASTER.md requires SCI / ΔH / ΔG to read the
    // same value in both themes, and the HUD reads the literals. Use an *Asset
    // accessor only where the surface should follow the system appearance.
    ...
    public static var fgMutedAsset: Color { Color("BrandFgMuted", bundle: .module) }
    /// Failure text sized for light surfaces; the catalog gained this pair with the twins.
    public static var stateFailTextAsset: Color { Color("BrandStateFailText", bundle: .module) }
}
